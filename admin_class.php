...
Class Action {
	...
	function login(){
		extract($_POST);
			$sorgu = $this->db->query("SELECT *,concat(lastname,', ',firstname,' ') as name FROM users where email = '".$email."' and password = '".md5($password)."' ");
		if($sorgu->num_rows > 0){
			foreach ($sorgu->fetch_array() as $key => $value) {
